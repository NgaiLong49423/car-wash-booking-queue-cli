...
- Hệ thống sẽ duyệt Waitlist theo thứ tự ưu tiên và tạm bỏ qua booking không vừa thời gian để xét booking phù hợp phía sau.
...
- Khi bấm hoàn tất:
  ...
  6. **Kiểm tra và bổ sung từ hàng chờ phụ**: Hệ thống duyệt Waitlist theo thứ tự ưu tiên; booking không vừa thời gian còn lại được bỏ qua tạm thời để tiếp tục xét booking phía sau. Mỗi booking vừa điều kiện được kéo lên cuối hàng chờ chính cho đến khi không còn booking phù hợp hoặc hết quỹ thời gian.
...
- Khi hủy booking:
  ...
  - Khi hủy booking trong slot chính của buổi hiện tại: Trạng thái booking chuyển sang `CANCELLED`. Hệ thống duyệt Waitlist theo ưu tiên để kéo booking phù hợp vào cuối hàng chờ chính.
  ...
  - Sau khi hủy booking `SERVING`, hệ thống duyệt Waitlist theo quy tắc thời gian còn lại và kéo các booking phù hợp lên cuối Main Queue.
...
- **Luồng xử lý chính**:
  ...
  7. Tính `remainingMinutes = periodTotalMinutes - usedMinutes`.
   8. Hệ thống duyệt Waitlist theo thứ tự ưu tiên; booking không đủ thời gian được bỏ qua tạm thời để tiếp tục xét các booking phía sau.
   9. Mỗi booking phù hợp được chuyển từ Waitlist vào cuối hàng chờ chính và `remainingMinutes` được cập nhật.
   10. Hệ thống dừng khi không còn booking phù hợp hoặc không còn thời gian.
...
- **Luồng xử lý chính**:
  ...
  6. Nếu booking bị hủy đang nằm trong Main Queue của buổi hiện tại, hệ thống loại booking khỏi Main Queue và duyệt Waitlist theo ưu tiên để bổ sung booking phù hợp nếu còn chỗ và thời gian.
  ...
  9. Sau khi hủy `SERVING`, hệ thống duyệt Waitlist theo quy tắc thời gian còn lại để kéo booking phù hợp lên Main Queue.
...
3. Hệ thống hiển thị thông tin cá nhân của Customer gồm: Mã khách hàng, Họ tên, Số điện thoại, Hạng thành viên, Điểm tích lũy, Tổng chi tiêu, Số lần rửa xe, Booking window, tiến độ lên hạng kế tiếp và quyền lợi Booking window sẽ được mở khóa.
...
- Nếu slot chính của buổi hiện tại bị trống, hệ thống duyệt Waitlist theo ưu tiên và đẩy booking phù hợp đầu tiên lên slot chính.
...
8. Hệ thống kiểm tra xem booking bị hủy có nằm trong slot chính của buổi hiện tại hay không:
   - Nếu đúng, hệ thống duyệt Waitlist theo ưu tiên và đưa booking phù hợp đầu tiên vào cuối hàng chờ chính (FIFO Queue).
...
10. Hệ thống duyệt Waitlist theo thứ tự ưu tiên và tạm bỏ qua booking không vừa thời gian để tiếp tục xét booking phía sau.
11. Mỗi booking phù hợp được chuyển từ Waitlist lên cuối Main Queue và cập nhật `remainingMinutes`.
12. Hệ thống dừng khi không còn booking phù hợp hoặc hết thời gian.
...
- Duyệt Waitlist theo thứ tự ưu tiên và bỏ qua tạm thời booking không vừa thời gian để tận dụng quỹ thời gian còn lại.
...
- Nếu mã khách hàng hợp lệ: Hệ thống xác định `currentCustomer = C001`, in lời chào bằng tiếng Anh và hiển thị Customer Menu:
...
2. View my profile
3. Create my booking
4. View my active bookings
5. Cancel my booking
6. View my wash history
0. Log out
```
...
3. Manage services
4. Manage bookings
5. Set current date/period
6. View history
7. Undo last completed booking
0. Back to Main Menu
```
